...
#define PLANET_FAILED_UNKNOW (100)        /**< 执行失败，未知错误 */
    ...
    */
    ...
    /*!
        ...
        \return 执行成功返回CAN_SUCCESS，失败返回其他
    */
    EXTERNFUNC int planet_getTargetCurrent(int devIndex, int id, float *current, int timeout = 100);
    ...
    */
    ...
    /*!
            ...
            \return 执行成功返回CAN_SUCCESS，失败返回其他
    */
    EXTERNFUNC int planet_setElectronicGearRatio(int devIndex, int id, float ratio, int timeout = 100);
    ...
    */
    ...
    /*!
        ...
        \param devIndex 设备索引，第一个设备设置为0，第二个设备设置为1，以此类推
        \param id 电机id
        \param velocity 存放读取的电机最大速度值（rpm）
        \param timeout 函数执行后，等待timeout毫秒来接收执行结果
        \return 执行成功返回CAN_SUCCESS，失败返回其他
    */
